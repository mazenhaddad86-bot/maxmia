...
# Komplett neue Phrasen, gleiche Thematik
LYRICS = """[Intro]
...
STYLE = "energetic upbeat children lullaby, joyful gentle kids song, bouncy magical melody, twinkling bells, soft strings, bright xylophone, playful brass accents, sweet child-friendly vocals, dreamy starlight feel, full 3 minute song"
...
with sync_playwright() as p:
    ...
    print(f"Shiny: {check}", flush=True)
    if isinstance(check, list):
        with open("scripts/shiny_ids.json","w") as f:
            json.dump({"theme":"twinkle","ids":check}, f)
