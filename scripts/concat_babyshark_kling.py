"""Concat BabyShark Kling clips + Suno audio."""
import subprocess, shutil
from pathlib import Path

PROJECT = Path(__file__).resolve().parent.parent
FFMPEG = shutil.which("ffmpeg") or r"C:\Program Files\WinGet\Links\ffmpeg.exe"
CLIPS = PROJECT / "output/babyshark/clips"
AUDIO = PROJECT / "output/babyshark/bs_93eec1c5.mp3"
OUT   = PROJECT / "output/babyshark/babyshark_kling_final.mp4"

order = sorted(CLIPS.glob("BS*.mp4"), key=lambda p: int(p.stem.replace("BS","")))
