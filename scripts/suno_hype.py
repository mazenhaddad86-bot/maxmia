...
# Gleiche ORIGINAL Lyrics (Copyright-safe)
LYRICS = """[Intro]
...
# HYPE Style — mehr Dance, Bass, Cool
STYLE = "kids pop dance hit, modern catchy children song, upbeat funky bassline, dance beat, joyful energetic, like Baby Shark style but cooler, 808 bass, bright synths, ukulele plucks, claps, catchy hook, 3 minutes full song"
...
with sync_playwright() as p:
    ...
    state = page.evaluate("""() => {
        ...
        return {top2: tiles.slice(0,2).map(a=>({h:a.getAttribute('href'),t:(a.textContent||'').trim().slice(0,40)})), loading: document.querySelectorAll('[class*="spin"]').length};
    }""")
